...
namespace PVE {
    ...
    public:
        ...
        PVE_EXPORT void SendSoundEvent(const std::string& name);
        ...
        PVE_EXPORT void RegisterCondition(const std::string& eventName, const std::string& conditionName, const std::function<std::variant<float, int, bool, std::string, RE::TESForm*>()>& conditionFunction);
        ...
        PVE_EXPORT void SetMemoryData(const std::string& key, const std::variant<std::string, int, float, RE::TESForm*>& value);

        // 1.0.3: Added
        // Retrieves a value from the Memory Data Storage. If the given key has no value, it returns 'def'
        // As the name suggests, this storage is only in memory, and will be flushed once the game is closed.
        PVE_EXPORT std::variant<std::string, int, float, RE::TESForm*> GetMemoryData(const std::string& key, const std::variant<std::string, int, float, RE::TESForm*>& def);
        ...
        PVE_EXPORT void SetSaveData(const std::string& key, const std::variant<std::string, int, float, RE::TESForm*>& value);

        // 1.0.3: Added
        // Retrieves a value from the Save Data Storage. If the given key has no value, it returns 'def'
        // This Storage is linked to the currently loaded Playthrough, and will be saved/loaded whenever the game is saved/loaded.
        // Therefore, Anything saved before a save is loaded will be discarded once a save is loaded.
        PVE_EXPORT std::variant<std::string, int, float, RE::TESForm*> GetSaveData(const std::string& key, const std::variant<std::string, int, float, RE::TESForm*>& def);

        // 1.0.3: Added
        // Returns whether or not the given form contains the given keyword.
        PVE_EXPORT bool FormHasKeyword(RE::TESForm* form, const std::string& keyword);

        // 1.0.3: Added
        // Returns the given form as a string-representation in the format 'PluginName|HexFormID', e.g. 'Skyrim.esm|0x123456'
        PVE_EXPORT std::string FormToString(const RE::TESForm* form);
        ...
        PVE_EXPORT RE::TESForm* FormFromString(const std::string& formString);

        // 1.0.3: Added
        // Returns the form associated to the given plugin name and Form ID.
        PVE_EXPORT RE::TESForm* FormFromID(const std::string& pluginName, const RE::FormID& formId);

        // 1.0.3: Added
        // Returns whether or not the two given forms are the same.
        PVE_EXPORT bool CompareForms(const std::string& first, const std::string& second);
        ...
        PVE_EXPORT bool CompareForms(const RE::TESForm* first, const std::string& second);

        // 1.0.3: Added
        // Returns the given form's attached keywords as string, in the format 'keyword1|keyword2|...'
        PVE_EXPORT std::string FormToKeywordString(RE::TESForm* form);

        // 1.0.3: Added
        // Returns the given string, with all occurences of 'oldSeq' replaced with 'newSeq'
        PVE_EXPORT std::string ReplaceInString(const std::string& text, const std::string& oldSeq, const std::string& newSeq);

        // 1.0.3: Added
        // Returns the given string with all leading and trailing whitespaces removed
        PVE_EXPORT std::string TrimString(const std::string& text);

        // 1.0.3: Added
        // Returns a vector that contains all parts of the given string split by the given delimiter char
        PVE_EXPORT std::vector<std::string> SplitString(const std::string& text, const char& delimiter);

        // 1.0.3: Added
        // Returns a random integer between 'minInclusive' and 'maxInclusive'
        PVE_EXPORT int RandomInt(int minInclusive, int maxInclusive);

        // 1.0.3: Added
        // Returns a random float between 'minInclusive' and 'maxInclusive'
        PVE_EXPORT float RandomFloat(float minInclusive, float maxInclusive);

    private:
        static std::unique_ptr<PlayerVoiceEventsAPI> Initialize();
    };

    inline std::mutex mutex;
    inline extern std::unique_ptr<PlayerVoiceEventsAPI> api_ptr = nullptr;
}