...
namespace PVE {
    void SoundManager::RegisterSoundEvent(const std::string& name, const int chance, const int cooldown, const std::vector<std::string>& overrideBlacklist, const std::vector<std::pair<std::string, std::vector<std::string>>>& audios) {
                ...
                if (!storage->Get(cooldownKey).AsInt()) {
                    ...
                    for (int i = 0; i < event.audios.size(); ++i) {
                        ...
                        if (const auto files = event.audios[i].second; !files.empty() && ConditionManager::GetSingleton()->EvaluateExpression(event.name, expr)) {
                            ...
                            std::string filePath;
                            const auto rngMode = MemoryDataStorage::GetSingleton()->Get("pve_rngMode", DataType("default"));
                            if (rngMode.AsString() == "default") {
                                filePath = std::format("Sound/PlayerVoiceEvents/SoundData/{}", files.at(Util::RandomInt(0, static_cast<int>(files.size()) - 1)));
                            } else if (rngMode.AsString() == "biased") {
                                ...
                                Logger::GetSingleton().LogDebug(rngMode.AsString());
                            }
                            ...
                            if (storage) {
                                storage->Set(cooldownKey, DataType(event.cooldown));
                            }
                            ...
                            while (this->currentHandle && this->GetCurrentSoundHandle()->GetDuration() <= 0) {
                                ...
                                delay += 5;
                                if (delay >= 2000) {
                                    // For some reason, on occasion the game does not seem to populate the 'Duration' field of a sound that gets played. This is especially an issue on very short sounds.
                                    // This is the exact same issue as with the 'voiceTimer' for dialogue.
                                    SKSE::log::warn("Last sound could not be detected as 'playing'. THIS IS AN ISSUE WITH THE GAME'S ENGINE ITSELF, DO NOT REPORT IT TO ME, I CAN'T FIX IT!!!");
                                    this->StopCurrentSoundEvent();
                                    return;
                                }
                            }
                ...
                if (const auto saveDataStorage = SaveDataStorage::GetSingleton()) {
                    for (auto name : this->registeredSoundEvents | std::views::keys) {
                        ...
                        if (const int intVal = saveDataStorage->Get(s).AsInt(); intVal > 0) {
                            saveDataStorage->ModifyIfExists(s, [](DataType data) {
