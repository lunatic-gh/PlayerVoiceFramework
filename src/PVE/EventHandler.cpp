...
namespace PVE {
        ...
        if (type == SKSE::ActionEvent::Type::kWeaponSwing) {
            ConditionManager::GetSingleton()->RegisterCondition("PVEAttackMelee", "AttackType", [playerActor]() {
                if (const auto currentProcess = playerActor->GetActorRuntimeData().currentProcess; currentProcess && currentProcess->high && currentProcess->high->attackData) {
                    if (const auto flags = currentProcess->high->attackData->data.flags; flags.any(RE::AttackData::AttackFlag::kPowerAttack)) return DataType(1);
                }
                return DataType(0);
            });
            ConditionManager::GetSingleton()->RegisterCondition("PVEAttackMelee", "WeaponType", [source]() {
                if (source && source->IsWeapon()) {
                    return DataType(static_cast<int>(source->As<RE::TESObjectWEAP>()->GetWeaponType()));
                }
                return DataType(0);
            });
            ConditionManager::GetSingleton()->RegisterCondition("PVEAttackMelee", "WeaponForm", [source]() { return DataType(source); });
            ConditionManager::GetSingleton()->RegisterCondition("PVEAttackMelee", "WeaponKeywords", [source]() { return DataType(FormUtil::ToKeywordString(source).c_str()); });
            ...
            if (source && source->Is(RE::FormType::Weapon)) {
                ConditionManager::GetSingleton()->RegisterCondition("PVEUnsheatheWeapon", "WeaponForm", [source] { return DataType(source); });
                ConditionManager::GetSingleton()->RegisterCondition("PVEUnsheatheWeapon", "WeaponKeywords", [source]() { return DataType(FormUtil::ToKeywordString(source).c_str()); });
            ...
            if (source && source->Is(RE::FormType::Spell)) {
                ConditionManager::GetSingleton()->RegisterCondition("PVEUnsheatheSpell", "SpellForm", [source] { return DataType(source); });
                ConditionManager::GetSingleton()->RegisterCondition("PVEUnsheatheSpell", "SpellKeywords", [source]() {
                    if (const auto spell = source->As<RE::SpellItem>()) {
                        return DataType(FormUtil::ToKeywordString(spell->GetAVEffect()).c_str());
                    }
                    return DataType("");
                });
            ...
            if (source && source->Is(RE::FormType::Weapon)) {
                ConditionManager::GetSingleton()->RegisterCondition("PVESheatheWeapon", "WeaponForm", [source] { return DataType(source); });
                ConditionManager::GetSingleton()->RegisterCondition("PVESheatheWeapon", "WeaponKeywords", [source]() { return DataType(FormUtil::ToKeywordString(source).c_str()); });
                SoundManager::GetSingleton()->SendSoundEvent("PVESheatheWeapon");
            }
        } else if (type == SKSE::ActionEvent::Type::kEndSheathe) {
            if (source && source->Is(RE::FormType::Spell)) {
                ConditionManager::GetSingleton()->RegisterCondition("PVESheatheSpell", "SpellForm", [source] { return DataType(source); });
    ...
    RE::BSEventNotifyControl PublicEventSink::ProcessEvent(const RE::TESPlayerBowShotEvent* a_event, RE::BSTEventSource<RE::TESPlayerBowShotEvent>* a_eventSource) {
        if (a_event) {
            ConditionManager::GetSingleton()->RegisterCondition("PVEAttackBow", "WeaponForm", [a_event] { return DataType(RE::TESForm::LookupByID(a_event->weapon)); });
            ConditionManager::GetSingleton()->RegisterCondition("PVEAttackMelee", "WeaponKeywords", [a_event]() { return DataType(FormUtil::ToKeywordString(RE::TESForm::LookupByID(a_event->weapon)).c_str()); });
            ConditionManager::GetSingleton()->RegisterCondition("PVEAttackBow", "DrawPower", [a_event] { return DataType(a_event->shotPower); });
    ...
    RE::BSEventNotifyControl PublicEventSink::ProcessEvent(const RE::TESDeathEvent* a_event, RE::BSTEventSource<RE::TESDeathEvent>* a_eventSource) {
        if (a_event && !a_event->dead) {
            if (const auto actor = a_event->actorDying; actor && actor->IsPlayerRef()) {
                ConditionManager::GetSingleton()->RegisterCondition("PVEDeath", "KillerForm", [a_event] { return DataType(a_event->actorKiller.get()); });
    ...
    RE::BSEventNotifyControl PublicEventSink::ProcessEvent(const RE::TESHitEvent* a_event, RE::BSTEventSource<RE::TESHitEvent>* a_eventSource) {
        ...
        ConditionManager::GetSingleton()->RegisterCondition("PVEHit", "IsPowerAttack", [a_event] { return DataType(a_event->flags.any(RE::TESHitEvent::Flag::kPowerAttack)); });
        ConditionManager::GetSingleton()->RegisterCondition("PVEHit", "IsBashAttack", [a_event] { return DataType(a_event->flags.any(RE::TESHitEvent::Flag::kBashAttack)); });
        ConditionManager::GetSingleton()->RegisterCondition("PVEHit", "IsAttackBlocked", [a_event] { return DataType(a_event->flags.any(RE::TESHitEvent::Flag::kHitBlocked)); });
        ConditionManager::GetSingleton()->RegisterCondition("PVEHit", "IsHealingAttack",
                                                            [cause, source] { return DataType(cause && cause->As<RE::Actor>() && cause->As<RE::Actor>()->IsPlayerTeammate() && source && source->Is(RE::FormType::Spell) && FormUtil::HasKeyword(source, "MagicRestoreHealth")); });
        if (!cause || !cause->As<RE::Actor>()) {
        ...
        if (const auto baseForm = RE::TESForm::LookupByID(a_event->spell)) {
            if (const auto spell = baseForm->As<RE::SpellItem>()) {
                if (const auto obj = a_event->object; obj && obj->IsPlayerRef()) {
                    ...
                    ConditionManager::GetSingleton()->RegisterCondition("PVESpellCast", "SpellForm", [spell] { return DataType(spell); });
                    ConditionManager::GetSingleton()->RegisterCondition("PVESpellCast", "SpellKeywords", [spell]() { return DataType(FormUtil::ToKeywordString(spell->GetAVEffect()).c_str()); });
